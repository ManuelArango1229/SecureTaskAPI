...
function authenticate(req, res, next) {
  ...
  try {
    // CORRECT: jwt.verify() validates the signature
...
app.post('/auth/register',
  ...
  async (req, res) => {
    ...
    // CORRECT: bcrypt hash, never store plaintext
...
app.post('/tasks',      authenticate,
  ...
  (req, res) => {
    ...
    if (!errors.isEmpty())
      return res.status(400).json({ errors: errors.array() });
    const task = { id: taskIdCounter++, title: req.body.title, owner: req.user.email };
